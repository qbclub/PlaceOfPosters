...
export default {
    ...
    getManagerPostersOnModeration(status,types,locations) {
        return $api.get(`/poster/get-manager-posters-on-moderation?status=${status}&types=${types}&locations=${locations}`)
    },
    ...
    getPostersMiniature(organizer,poster_id, posterStatus) {
        return $api.get(`/poster/get-posters-miniature?organizer=${organizer}&poster_id=${poster_id}`)
    },
